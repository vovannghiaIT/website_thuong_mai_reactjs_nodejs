...
const HomeOpera = () => {
  ...
  const [dataOperaEdit, setDataOperaEdit] = useState({});
  ...
  const submitStatus = (items) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await apiUpdateOpera({ ...items, status: 0 });
        fetchData();
      }
    });
  };
